...
public class CabService {
	...
	public String bookRide(@RequestBody RideRequest rideRequest)
	{
		...
		List<CabDTO> nearByDrivers = findAllCabsNearby(rideRequest.getSourceLat(),rideRequest.getSourceLon(),RADIUS);
	
		if(isRideAccepted(rideRequest.getUserId())) {
			return "ride has been already accepted"; 
		}
		
		for(CabDTO driver : nearByDrivers) {
			sendNotification(driver.getDriverId().toString(), rideRequest);
		}
